...
class OMFastSearchSource(mxs:MatrixStore, vaultId:String, searchTerms:Array[SearchTerm], includeFields:Array[String], contentSearchBareTerm:Boolean=false, atOnce:Int=10) extends GraphStage[SourceShape[ObjectMatrixEntry]] {
  ...
  override def createLogic(inheritedAttributes: Attributes): GraphStageLogic = new GraphStageLogic(shape) {
    ...
    override def preStart(): Unit = {
      ...
      try {
        ...
        logger.info(s"Establishing connection to $vaultId")
        vault = Some(mxs.openVault(vaultId))
